...
└──────────────────────────┬──────────────────────────────────┘
         ...
         │                 │                 │
    ┌────▼────┐      ┌─────▼─────┐    ┌──────▼──────┐
    │Fırsatçı │      │Tedarikçi  │──proforma──►│  Listeleme  │
    │0 6,18   │      │0 * * * *  │  (sourced)  │  (M5+)      │
    │* * *    │      │(saatte 1) │             └──────┬───────┘
    └────┬────┘      └─────┬─────┘                    │
         │                 │                  ┌────────┴────────┐
...
> **Tedarikçi──proforma──►Listeleme:** GAP-2 ile proforma teklif akışı implemente
> edildi (tedarikci.py Faz 5, proforma_offers tablosu, products.status='sourced'
> geçişi). Listeleme Agent'ın Shopify/Trendyol API entegrasyonu **M5 öncesi
> tamamlanacak**.
